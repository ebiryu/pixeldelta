// Measures pixeldelta against pixelmatch and odiff on the committed benchmark
// fixtures, on one machine. Returns rows of structured data; nothing here
// prints. bench.mjs formats the rows into tables, and record.mjs folds them
// into a BENCHMARKS.md entry.
//
//   pnpm install
...
export const SIZES = ['2mpx', '8mpx', '18mpx'];
...
export const PROCESS_RUNS = 10;
export const ENGINE_RUNS = 50;
...
/** Engine-only median milliseconds and diff count for pixelmatch on decoded buffers. */
function pixelmatchEngine({ a, b }, aa) {
  const samples = [];
  let diff = 0;
  for (let i = 0; i < ENGINE_RUNS; i++) {
    const start = performance.now();
    const count = pixelmatch(a.data, b.data, null, a.width, a.height, {
      threshold: THRESHOLD,
      includeAA: aa !== 'on',
    });
    samples.push(performance.now() - start);
...
/**
 * Engine-only rows: the compare of decoded RGBA buffers, for pixeldelta and
 * pixelmatch, one row per size and anti-aliasing setting. odiff has no buffer
 * entry point and is left out.
 */
export function engineRows() {
  ...
  for (const size of SIZES) {
    ...
    for (const aa of ['off', 'on']) {
      const pixeldeltaMs = pixeldeltaEngine(size, aa);
      const pixelmatchResult = pixelmatchEngine(buffers, aa);
      rows.push({
        size,
        aa,
        pixeldelta: pixeldeltaMs,
        pixelmatch: pixelmatchResult.ms,
        pixeldeltaDiff: pixeldeltaDiff(size, aa),
        pixelmatchDiff: pixelmatchResult.diff,
      });
    }
  }
  return rows;
...
/**
 * End-to-end rows: two PNG files to a verdict, decode included, no diff
 * image, as the wall-clock time of the process, one row per size and
 * anti-aliasing setting.
 */
export function endToEndRows() {
  const rows = [];
  for (const size of SIZES) {
    const dir = join(fixtures, size);
    ...
    for (const aa of ['off', 'on']) {
      ...
      const pixeldeltaMs = timeProcess(() => run(pixeldelta, pdArgs), PROCESS_RUNS);
      const pixelmatchMs = timeProcess(() => run(process.execPath, pmArgs), PROCESS_RUNS);
      let odiffDiff = 0;
      const odiffMs = timeProcess(() => {
        odiffDiff = runOdiff(base, head, aa);
      }, PROCESS_RUNS);
      rows.push({
        size,
        aa,
        pixeldelta: pixeldeltaMs,
        pixelmatch: pixelmatchMs,
        odiff: odiffMs,
        odiffDiff,
      });
    }
...
/** Each tool's startup floor, in median milliseconds on a one-pixel pair. */
export function startupFloor() {
